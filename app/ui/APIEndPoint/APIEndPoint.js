  ...
  const fetchEndpoints = async () => {
    try {
      ...
      if (response.data && response.data.data) {
        ...
        // Initialize editing state for each record
        ...
        <table className={styles.table}>
          ...
          <tbody>
            {Array.isArray(data) && data.map((record, index) => (
              <tr key={record.isNew ? `new-${index}` : `${record.menu_id}-${record.endpoint}`}>
                <td>
                  <input
                    type="text"
                    className={styles.input}
                    value={editingData[index]?.menu_id || ''}
                    onChange={(e) => handleInputChange(index, 'menu_id', e.target.value)}
                  />
                </td>
                <td>
                  <input
